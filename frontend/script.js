...
const getImageDimensions = (aspectRatio, baseSize = 512) => {
    ...
    const scaleFactor =
        baseSize / Math.sqrt(width * height);
    ...
    calculatedWidth =
        Math.floor(calculatedWidth / 16) * 16;

    calculatedHeight =
        Math.floor(calculatedHeight / 16) * 16;

    if (calculatedWidth < 16) calculatedWidth = 16;
...
promptBtn.addEventListener("click", () => {
    ...
    const prompt =
        examplePrompts[
            Math.floor(
                Math.random() * examplePrompts.length
            )
        ];
